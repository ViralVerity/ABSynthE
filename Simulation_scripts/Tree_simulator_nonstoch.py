#Will need to set current_run and dropbox_path before using this - maybe have it as inputs to the overall function?
...
def sampling(lst, sampled_percentage): 
    """Get who is sampled, inputs are list of individual ids and the sampled percentage"""
    
    #print(sampled_percentage)
    
    total = len(lst) #Doesn't have NA in it because that can't be sampled
    
    number_of_people = round(total*sampled_percentage)
    
    random.seed(seed_no)
    those_sampled = set(random.sample(lst, k = number_of_people))
    ...
    for nde, height in sorted_dict.items():
        ...
        for number, times in coalescent_intervals.items():            
            if not nde.node_parent:
                ...
                if height < times[1] and whole_tree.final_node_heights[whole_tree.root] >= times[1]:
...
def calculate_ne(whole_tree):
    ...
    for key, value in waiting_times.items():
        ...
        new_key = coalescent_intervals[key][1] 
...
def simulate_tree(trans_dict, nodes, sampling_proportion): #change these arguments when putting in module for reading in
    """Function to simulate a coalescent tree"""
            
    those_sampled = sampling(nodes, sampling_proportion)
    ...
    if len(those_sampled) != 0:
        ...
        Ne_dict = calculate_ne(big_tree)

        return newick_tree, Ne_dict, big_tree, R0
